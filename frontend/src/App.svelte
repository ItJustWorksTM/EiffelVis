...
<script lang="ts">
  ...
  let qhistory: { stream: QueryStream; query: Query }[] = [];

  let testingq: FixedQuery = {
  ...
  const submit_state_query = () => submit_query(fixed_query_to_norm(testingq));

  const submit_query = (new_query: Query) => {
    active_stream = (() => {
      const cached = qhistory.find((v) => query_eq(new_query, v.query));
      if (cached) {
        return cached.stream;
      } else {
        const ret = new QueryStream(connection, deep_copy(new_query));
        qhistory = [...qhistory, { stream: ret, query: deep_copy(new_query) }];
        return ret;
      }
    })();
  ...
  const use_selected_as_root = () => {
    testingq.collection = { type: "AsRoots" };
    testingq.range_filter = { begin: null, end: null };

    const hm = empty_fixed_event_filters();
    hm.ids.pred.ids = [selected_node.meta.id];
    testingq.event_filters = [hm];
    ...
    <div class="container h-full w-full p-1 overflow-hidden scroll-auto">
      ...
      <div class="btn-group w-full flex flex-row mt-2">
        ...
        <button
          ...
          class:btn-disabled={qhistory.length <= 1 || awaiting_query_request}
          on:click={() => {
        ...
        <button
          ...
          class:btn-disabled={awaiting_query_request}
