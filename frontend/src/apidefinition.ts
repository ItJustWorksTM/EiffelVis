...
export interface FullEvent {
    meta: {
        ...
        time: number;
        tags?: string[];
        source?: { host: string; name: string };
    };
    data: {};
...
export const string_compare_eq = (lhs: StringCompare, rhs: StringCompare) =>
    lhs.value == rhs.value && lhs.lower_case == rhs.lower_case && lhs.partial == rhs.partial;

export const string_compare_default = (): StringCompare => {
    return { value: '', lower_case: false, partial: false };
};

export type Type = TypeTag<
    'Type',
    {
        names: StringCompare[];
    }
...
export type SourceHost = TypeTag<
    'SourceHost',
    {
        hosts: StringCompare[];
    }
...
export type SourceName = TypeTag<
    'SourceName',
    {
        names: StringCompare[];
    }
...
export const event_filter_eq = (
    lhs: EventFilter<EventFilterType>,
    rhs: EventFilter<EventFilterType>,
...
export const range_filter_eq = (lhs: RangeFilter, rhs: RangeFilter): boolean =>
    ((!lhs.begin && !rhs.begin) ||
        (lhs.begin && rhs.begin && range_filter_bound_eq(lhs.begin, rhs.begin))) &&
