...
const backend_url: string = process.env.EIFFELVIS_URL.startsWith('@origin')
    ? `${window.location.host}${window.location.pathname}${process.env.EIFFELVIS_URL.replace(
          '@origin',
          '',
      )}`
...
const connection: EiffelVisConnection = new EiffelVisConnection(backend_url, backend_has_ssl);
